#!/usr/bin/env bash
# 等 800 checkpoint 写完整 -> 停当前 ODE 训练 -> 关 cpu_offload + resume 800 -> 相同方式重启续训。
...
log "watcher 启动, 等待 800 checkpoint ($CKPT_PT) ..."
...
log "800 checkpoint 写入完成, size=$prev bytes"
...
# 4) 改 config: 关 cpu_offload(提速), resume 800
sed -i "s|^resume_ckpt:.*|resume_ckpt: logs/bernini_v2v_ode_routeb/checkpoint_model_000800/model.pt|" "$CFG"
sed -i "s|^ckpt_step:.*|ckpt_step: 800|" "$CFG"
sed -i "s|^generator_cpu_offload:.*|generator_cpu_offload: false  # 95GB 卡余量充足, 关闭 offload 去掉 CPU<->GPU 搬运以提速|" "$CFG"
log "config 已更新: generator_cpu_offload=false, resume_ckpt=800, ckpt_step=800"
...
echo "" >> "$TRAIN_LOG"
echo "==================== RESTART @ $(date '+%F %T') : resume 800(local-cache), cpu_offload=OFF ====================" >> "$TRAIN_LOG"
log "重启 ODE 训练 (8 卡, 关 offload, 从 800 本地缓存续训)..."
nohup "$PY" -m torch.distributed.run --nnodes=1 --nproc_per_node=8 \
  --rdzv_id=8843 --rdzv_backend=c10d --rdzv_endpoint=localhost:29542 \
